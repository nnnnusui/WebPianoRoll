...
const Grid: React.FC<Props> = () => {
  ...
  const forward = () => {
    ...
    if (roll == null) return;
    const cellSize = {
      width: (canvas.width / roll.width) * scale,
      height: (canvas.height / roll.height) * scale,
    };
    ...
    const onWheel = (event: React.WheelEvent) => {
      ...
      setScaleCount((prev) => {
        ...
        const mouse = getElementLocalMousePosFromEvent(event);
        const ratio = {
          width: (move.state.x + mouse.x) / (canvas.width * scale),
          height: (move.state.y + mouse.y) / (canvas.height * scale),
        };
        const scalingVector = {
