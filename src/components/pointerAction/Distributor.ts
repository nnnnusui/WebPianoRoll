...
const Conditions = (pointers: State) => {
  return {
    hold: (milliSec: number) => (from: Event) => (to: Event) => {
      const difference = to.timeStamp - from.timeStamp;
      return milliSec > difference;
    },
  };
};
...
const Distance = (upper: number): Condition => (from: Event) => (to: Event) => {
...
const RadialMenu = (factories: ExecutorFactory[]) => (from: Event) => (
  to: Event
) => {
  const division = factories.length;
  const unit = 360 / division;
  const shift = unit / 2;
  const conditions = factories.map((factory, index) => ({
...
const Tree = RadialMenu([Log("up"), Log("right"), Log("down"), Log("left")]);
